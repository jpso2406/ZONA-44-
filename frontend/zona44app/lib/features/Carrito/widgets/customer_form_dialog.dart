  ...
  void _validateForm() {
    setState(() {
      ...
      if (nameController.text.trim().isEmpty) {
        _errors['name'] = AppLocalizations.of(context)!.nameRequired;
      } else if (nameController.text.trim().length < 3) {
        _errors['name'] = AppLocalizations.of(context)!.nameTooShort;
      }

      final emailRegex = RegExp(r'^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$');
      if (emailController.text.trim().isEmpty) {
        _errors['email'] = AppLocalizations.of(context)!.emailRequired;
      } else if (!emailRegex.hasMatch(emailController.text.trim())) {
        _errors['email'] = AppLocalizations.of(context)!.invalidEmail;
      }

      if (phoneController.text.trim().isEmpty) {
        _errors['phone'] = AppLocalizations.of(context)!.phoneRequired;
      } else if (phoneController.text.replaceAll(RegExp(r'[^\d]'), '').length <
          7) {
        _errors['phone'] = AppLocalizations.of(context)!.invalidPhone;
      }

      if (deliveryType.value == 'domicilio') {
        if (addressController.text.trim().isEmpty) {
          _errors['address'] = AppLocalizations.of(context)!.addressRequired;
        } else if (addressController.text.trim().length < 10) {
          _errors['address'] = AppLocalizations.of(context)!.addressTooShort;
        }

        if (cityController.text.trim().isEmpty) {
          _errors['city'] = AppLocalizations.of(context)!.cityRequired;
        } else if (cityController.text.trim().length < 3) {
          _errors['city'] = AppLocalizations.of(context)!.cityTooShort;
        }
      }
        ...
        Container(
          decoration: BoxDecoration(
            gradient: LinearGradient(
              colors: hasError
                  ...
                  : [
                      Color(0xFF040E3F).withOpacity(0.6),
                      Color(0xFF0A2E6E).withOpacity(0.4),
                    ],
              begin: Alignment.topLeft,
              end: Alignment.bottomRight,
            ),
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              ...
              hintText: AppLocalizations.of(context)!.enterYour(label),
